...
namespace pelsoft.auth.Amazon.ldap
{
     public  class LDAPHelper : DirectoryServicesBase
    {
        ...
        public LDAPHelper(string domainName, string connStringName)
        {
            Init(domainName, connStringName, false);

        }
        void Init(string domainName, string connStringName, bool chekControllers)
        {
            ...
            _DomainUrlInfo = DirectoryServicesBase.DomainsUrl_Get_FromSp(connStringName, domainName);// _DomainUrlInfoList.First<DomainUrlInfo>(p => p.DomainName == domainName);
        ...
        /// </summary>
        /// <param name="te"></param>
        private static void SetError(TechnicalException te)
        {
        ...
        DirectoryEntry GetImpersonate_SearchRoot_DE()
        {
            ...
            catch (Exception e)// Cuando el usuario no existe o clave erronea
            {
                ...
                TechnicalException te = new TechnicalException(string.Format("Impersonalization error.- The security Active directory throw  the following error : {0}", te1.Message), te1.InnerException);
    ...
    /// </summary>
    public class ADWrapper : DirectoryServicesBase
    {
        ...
        private void Init_withoutdatabase(string domainName, bool performWindowsContextImpersonalization)
        {
            ...
            try
            {
                ...
                _LDAPDomainName = get_DISTINGUISHEDNAME(_directoryEntrySearchRoot);
            }
            catch (TechnicalException e)// Cuando el usuario no existe o clave erronea
            {
                Exception te1 = ProcessActiveDirectoryException(e);
                TechnicalException te = new TechnicalException(string.Format("Error de impersonalización", te1.Message), te1.InnerException);
                ExceptionHelper.SetTechnicalException<ADWrapper>(te);
                te.ErrorId = "4103";
                throw te;
            }
        ...
        /// <param name="userName"></param>
        /// <returns></returns>
        public DirectoryEntry Person_Get(string userName)
        {
        ...
        public bool User_IsAccountLocked(string userName)
        {
            ...
            //The 514 value coming from NORMAL_ACCOUNT(512) + ACCOUNTDISABLE(2). UF_ACCOUNTDISABLE = 0x0002,
            ...
            try
            {
                ...
                wIsAccountLocked = User_IsAccountLockout(userDirectoryEntry);
                userDirectoryEntry.Close();
                return wIsAccountLocked;
        ...
        public void User_ResetPwd(string userName, string password, bool unlockAccount, int? pwdLastSet = -1)
        {
            ...
            try
            {
                ...
                if (userDirectoryEntry == null)
                {
                    ...
                    strErr.AppendLine(string.Format("No existe el usuario {0}", userName));
        ...
        static readonly string DomaunAndUserFormat = @"{0}\{1}";
        /// <summary>
        /// Obtiene un usuario por nombre Llama al User_Get el cual aplica :
        /// deSearch.Filter = string.Format("(&(ObjectClass={0})(sAMAccountName={1}))", "person", userName);
        /// </summary>
        /// <param name="userName">puede ser con o sin dominio siempre se le aplica FilterOutDomain</param>
        /// <returns></returns>
        ...
        /// Intermnamente realiza un deSearch.Filter = string.Format("(&(ObjectClass={0})(sAMAccountName={1}))", "person", userName);
        /// </summary>
        /// <param name="userName">Nombre de loging de usuario No aplica FilterOutDomain</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// adADWrapper.GetRootProperty(ADProperties.DESCRIPTION);
        /// </summary>
        /// <param name="prop"></param>
        /// <returns></returns>
        public string GetRootProperty(string prop)
        {
            string str1 = GetProperty(_directoryEntrySearchRoot, prop.ToString());

            //string propertie = Enum.GetName(typeof(ADProperties), prop);

            string str2 = GetValue(GetProperty(_directoryEntrySearchRoot, prop), "DC");
            return str2;
        }
        /// <summary>
        /// 
        ...
        static string GetStringProperties(DirectoryEntry pDirectoryEntry)
        {
            ...
            foreach (string s in pDirectoryEntry.Properties.PropertyNames)
            {
                slist.Append(s);
                slist.Append(" = ");
        ...
        string get_DISTINGUISHEDNAME(DirectoryEntry de)
        {
            string str = GetProperty(de, ADProperties.DISTINGUISHEDNAME);
            str = str.Replace(",", ".").Replace("DC=", "");
            return str;
